...
public class StudentService : IStudentService
{
    ...
    public void Add(Student entity)                => _repo.Add(entity);
    ...
    public List<Student> Search(string query)
    {
        ...
        return GetAll().Where(s => s.FullName.Contains(query, StringComparison.OrdinalIgnoreCase)).ToList();
    }
}
...
public class CourseService : ICourseService
{
    ...
    public CourseService(INotificationService notifService, IStudentService studentService)
    {
        ...
        Add(new Course { Code = "CS-284", Title = "Visual Programming",      CreditHours = 3, MaxCapacity = 30, FacultyId = faculty1Id });
        Add(new Course { Code = "CS-301", Title = "Data Structures",         CreditHours = 3, MaxCapacity = 25, FacultyId = faculty1Id });
        Add(new Course { Code = "CS-401", Title = "Software Engineering",    CreditHours = 3, MaxCapacity = 35, FacultyId = faculty1Id });
        Add(new Course { Code = "MT-201", Title = "Calculus-II",             CreditHours = 3, MaxCapacity = 40 });
        Add(new Course { Code = "CS-302", Title = "Computer Networks",       CreditHours = 3, MaxCapacity = 30 });
    }

    public List<Course> GetAll()           => _repo.GetAll();
    public Course? GetById(Guid id)        => _repo.GetById(id);
    public void Add(Course entity)         => _repo.Add(entity);
    ...
    public void EnrollStudent(Guid studentId, Guid courseId)
    {
        ...
        var student = _studentService.GetById(studentId) ?? throw new Exception("Student not found.");

        if (course.Status == EnrollmentStatus.Full)
            throw new CourseFullException(course.Title); // caught in the component

        // Business rule: can't re-enroll if dropped this semester
        if (student.Enrollments.Any(e => e.CourseId == courseId && e.DroppedThisSemester))
            throw new Exception("You cannot re-enroll in a course you dropped this semester.");

        // Already enrolled?
        if (student.Enrollments.Any(e => e.CourseId == courseId && e.State == EnrollmentState.Active))
    ...
    {
        var student = _studentService.GetById(studentId) ?? throw new Exception("Student not found.");
        var enrollment = student.Enrollments.FirstOrDefault(e => e.CourseId == courseId && e.State == EnrollmentState.Active)
            ?? throw new Exception("Enrollment not found.");

        // Business rule enforced in service, not component
        ...
        enrollment.DroppedThisSemester = true;

        var course = GetById(courseId);
        if (course != null)
        {
            var courseEnrollment = course.Enrollments.FirstOrDefault(e => e.StudentId == studentId && e.State == EnrollmentState.Active);
            if (courseEnrollment != null) courseEnrollment.State = EnrollmentState.Dropped;
        }
        OnEnrollmentChanged?.Invoke();
    }
    ...
    public List<Course> GetAvailableCourses(Guid studentId)
    {
        ...
        return GetAll().Where(c =>
            ...
            !student.Enrollments.Any(e => e.CourseId == c.Id && e.State == EnrollmentState.Active)
        ).ToList();
    }
    ...
    public List<Course> GetEnrolledCourses(Guid studentId)
    {
        var student = _studentService.GetById(studentId);
        if (student is null) return new();
        var enrolledIds = student.Enrollments
            .Where(e => e.State == EnrollmentState.Active)
            .Select(e => e.CourseId).ToList();
        return GetAll().Where(c => enrolledIds.Contains(c.Id)).ToList();
    }
...
public class GradeService : IGradeService
{
    ...
    public GradeService(INotificationService notifService, IStudentService studentService, ICourseService courseService)
    {
        _notifService   = notifService;
        _studentService = studentService;
    ...
    {
        var existing = _grades.FirstOrDefault(g => g.StudentId == record.StudentId && g.CourseId == record.CourseId);
        if (existing != null) { existing.Marks = record.Marks; existing.CreditHours = record.CreditHours; }
        else                  _grades.Add(record);

        // Update student CGPA after grade submission
        var student = _studentService.GetById(record.StudentId);
        if (student != null) student.CGPA = ComputeCGPA(record.StudentId);
    ...
    // Weighted CGPA = Σ(GradePoints × CreditHours) / Σ(CreditHours)
...
// Event-driven: components subscribe/unsubscribe — no polling
public class NotificationService : INotificationService
{
    private readonly List<Notification> _notifications = new();

    // C# event — NotificationBell subscribes in OnInitialized, unsubscribes in Dispose
    public event Action<Notification>? OnNewNotification;

    public void Send(Notification notification)
    {
        _notifications.Add(notification);
        OnNewNotification?.Invoke(notification); // broadcast to all subscribers
    }

    public List<Notification> GetForUser(Guid userId)
        => _notifications.Where(n => n.UserId == userId).OrderByDescending(n => n.CreatedAt).ToList();
